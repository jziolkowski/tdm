...
from GUI.UpdateDevices import UpdateDevicesDialog
...
exportOptions = [
    "1: Device name only (CSV)                       ",
    "2: Device summary data (CSV)                    ",
    "3: Device detailed data (json)                  ",
    "4: Device list for Batch Processing (CSV)       "
]
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowIcon(QIcon("icons/cricketwireless.png"))
        self.setWindowTitle("Cricket Wireless Store Traffic IOT Device Manager {}".format(self._version))
    ...
    def build_mainmenu(self):
        ...
        mMQTT.addAction(QIcon("icons/server-cloud.png"), "Broker", self.setup_broker)
       
        mMQTT.addSeparator()
        mMQTT.addAction(QIcon("icons/compass--plus.png"), "Autodiscovery patterns", self.patterns)
        ...
        self.mUtilities = self.menuBar().addMenu("Utilities")
        self.mUtilities.setEnabled(False)
        self.mUtilities.addAction(QIcon("icons/database-export.png"), "Export Device Data", self.export)
        self.mUtilities.addAction(QIcon("icons/compile.png"), "Update Devices", self.updateDevices)     
        
        mSettings = self.menuBar().addMenu("Settings")
        mSettings.addAction(QIcon("icons/wi-fi-zone.png"), "BSSId aliases", self.bssid)
        mSettings.addSeparator()
        mSettings.addAction(QIcon("icons/hammer-screwdriver.png"), "Preferences", self.prefs)
        ...
        if device:
            ...
            if topic.endswith("LWT"):
                ...
                if topic.endswith("FULLTOPIC"):
                    ...
                    if full_topic:
                        ...
                        if parsed:
                            ...
                            if d:
                                ...
                                logging.debug("DISCOVERY: Sending initial query to topic %s", topic)
                                self.initial_query(d, True)
                    ...
                    # Get devicename from topic;
                    if topic.endswith("STATE"):  
                        d = topic[topic.find('/')+1 : topic.find('/S')]
                    else:
                        d = topic[topic.find('/')+1 : topic.find('/R')]
                    
                    # Set topic to defaul of the device name and full topic to it's default
                    topic = d
                    full_topic = '%prefix%/%topic%/'

                    d = TasmotaDevice(topic, full_topic, d)
                    
                    # unknown device came online, set topic based on devicename and full topic to topic provided
                    logging.info("DISCOVERY: Discovered topic=%s with fulltopic=%s", topic, full_topic)
    ...
    def export(self):
        cwExport, ok = QInputDialog.getItem(self, "Device Data Export", "Select device data to export", exportOptions, editable=False)
        if ok:
            fname, _ = QFileDialog.getSaveFileName(self, "Export device list as...", directory=QDir.homePath(), filter="All files (*)")
            if fname:
                selection = cwExport.split(":")[0]
    ...
    def updateDevices(self):
        UpdateDevicesDialog(self).exec_()
